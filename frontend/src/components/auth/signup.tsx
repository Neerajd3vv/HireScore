...
export default function SignUpForm({ setShowSignUpModal, setShowSignInModal }: { setShowSignUpModal: React.Dispatch<React.SetStateAction<boolean>>; setShowSignInModal: React.Dispatch<React.SetStateAction<boolean>>; }) {
    ...
    return (
        <Card className="w-full max-w-lg border border-white/10 bg-gradient-to-bl from-[#000000] via-[#000814] to-[#000000]">
            <CardHeader className="space-y-1 text-center pb-6">
                ...
                <CardTitle className="text-2xl text-[#edf6f9]">Create account</CardTitle>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1">
                            <Input type="text" placeholder="First name" {...register("firstname")} className="h-11 text-[#edf6f9] border border-white/20" />
                            {errors.firstname && <p className="text-red-500 text-sm">{errors.firstname.message}</p>}
                        </div>

                        <div className="space-y-1">
                            <Input type="text" placeholder="Last name" {...register("lastname")} className="h-11 text-[#edf6f9] border border-white/20" />
                            {errors.lastname && <p className="text-red-500 text-sm">{errors.lastname.message}</p>}
                        </div>
                    </div>

                    <div className="space-y-1">
                        <Input type="email" placeholder="Email address" {...register("email")} className="h-11 text-[#edf6f9] border border-white/20" />
                    ...
                    <Button
                        className="w-full h-11 cursor-pointer bg-gradient-to-tr from-[#0a0a0f] via-[#111827] to-[#0f172a] text-white font-medium hover:shadow-[0_0_30px_rgba(56,189,248,0.5)]
                            hover:from-cyan-600 hover:via-blue-700 hover:to-indigo-800
                            hover:border-none transition-all duration-300"
                        type="submit"
                        disabled={isSubmitting}
                    >
                        {isSubmitting ? <Loader2 className="animate-spin w-6 h-6" /> : "Create account"}
                    </Button>
                </form>
